...
# Stage 4: build the KCR app
FROM node:22-alpine AS kcr-builder
WORKDIR /kcr
COPY kcr-app/package*.json ./
RUN npm ci
COPY kcr-app/ ./
RUN npm run build

# Stage 5: production backend (serves API + all frontends)
FROM node:22-alpine
WORKDIR /app
...
COPY --from=kcr-builder  /kcr/dist      ./public-kcr
EXPOSE 3001
