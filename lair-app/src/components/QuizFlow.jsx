...
function matchCorrectness(q, raw) {
  ...
  const result   = {};
  Object.entries(raw ?? {}).forEach(([src, tgt]) => { result[src] = matchMap[src] === tgt; });
  return result;
}

function MultipleChoice({ q, shuffledOpts, selected, onToggle, revealed, forced }) {
  ...
  return (
    <div className="qz-options">
      {shuffledOpts.map((opt) => {
        ...
        return (
          <button key={opt.id} className={cls} onClick={() => !(revealed || forced) && onToggle(opt.id)} disabled={revealed || forced}>
            <span className="qz-opt-marker">{multi ? '☐' : '○'}</span>
            {isSelected && !(revealed || forced) && <span className="qz-opt-marker qz-opt-filled">{multi ? '☑' : '●'}</span>}
  ...
  return (
    <div className="qz-tf-row">
      {[true, false].map((val) => {
        ...
        return (
          <button key={label} className={cls} onClick={() => !(revealed || forced) && onSelect(val)} disabled={revealed || forced}>
            {label}
          </button>
        );
      })}
...
function DragMatch({ q, matchState, onMatch, revealed, forced, partialFeedback }) {
  ...
  const handleDrop = (targetId) => { if (!dragging || locked) return; onMatch({ ...assigned, [dragging]: targetId }); setDragging(null); };
  const handleSourceClick = (srcId) => { if (locked) return; if (pending === srcId) { setPending(null); return; } setPending(srcId); };
  const handleTargetClick = (tgtId) => { if (locked || !pending) return; onMatch({ ...assigned, [pending]: tgtId }); setPending(null); };
  const clearSlot = (srcId) => { if (locked) return; const next = { ...assigned }; delete next[srcId]; onMatch(next); };
    ...
    <div className="qz-dragmatch">
      <div className="qz-dm-col">
        ...
        {q.payload.sources.map((src) => {
          ...
          return (
            <div key={src.id} className={cls} draggable={!locked && !isMapped}
              onDragStart={() => setDragging(src.id)} onDragEnd={() => setDragging(null)}
      ...
      <div className="qz-dm-col">
        ...
        {q.payload.targets.map((tgt) => {
          ...
          if (locked && placedSrcId) slotCls += matchMap[placedSrcId] === tgt.id ? ' qz-correct-opt' : ' qz-wrong-opt';
          return (
            <div key={tgt.id} className={slotCls} onDragOver={(e) => e.preventDefault()} onDrop={() => handleDrop(tgt.id)} onClick={() => handleTargetClick(tgt.id)}>
          ...
          {q.payload.matches.map((m) => {
            ...
            return (
              <div key={m.sourceId} className="qz-dm-key-row">
                <span>{src?.text}</span><span className="qz-dm-arrow">→</span><span>{tgt?.text}</span>
              </div>
            );
          })}
        </div>
  ...
  return (
    <div className="qz-fillblank">
      <input
        ...
        placeholder="Type your answer…" disabled={locked}
      />
      {forced && <div className="qz-forced-answer">Accepted: {q.payload.blanks?.[0]?.accepted?.join(' / ')}</div>}
    </div>
  );
}
...
export default function QuizFlow({ questions, assignmentId, color, onComplete }) {
  const shuffledOpts = useMemo(() =>
    questions.map((q) =>
      q.payload.kind === 'multiple_choice' && q.payload.shuffle ? shuffle(q.payload.options) : (q.payload.options ?? [])
    ), [questions]);
  ...
  const [qStates, setQStates] = useState(() =>
    Object.fromEntries(questions.map((q) => [q.id, {
      available: q.scoring.points, hintUsed: false, hintVisible: false,
      revealed: false, forced: false, lastWrong: false, partialFeedback: null,
    }]))
  );
  ...
  const updateAnswer = useCallback((val) => { setAnswers((a) => ({ ...a, [q?.id]: val })); }, [q?.id]);
  const toggleOption = useCallback((optId) => {
    const current = answers[q?.id] ?? [];
    if (q?.payload.selectionMode === 'single') updateAnswer([optId]);
    else updateAnswer(current.includes(optId) ? current.filter((id) => id !== optId) : [...current, optId]);
  ...
  const handleHint = useCallback(() => {
    setQStates((s) => {
      ...
      return { ...s, [q.id]: { ...cur, hintUsed: true, hintVisible: true, available: newAvail, forced: newAvail === 0 } };
    });
  }, [q?.id]);
  ...
  const handleSubmit = useCallback(() => {
    ...
    if (correct) {
      ...
      updateProgress(assignmentId, Math.round(((answered + 1) / questions.length) * 100)).catch(() => {});
    } else {
      setQStates((s) => {
        const cur      = s[q.id];
        ...
        return { ...s, [q.id]: { ...cur, available: newAvail, forced: doForce, lastWrong: !doForce, partialFeedback: q.payload.kind === 'drag_match' ? matchCorrectness(q, raw) : null } };
      });
      if (qs.available - 2 <= 0) updateProgress(assignmentId, Math.round(((answered + 1) / questions.length) * 100)).catch(() => {});
    }
  }, [q, raw, qs, answered, questions.length, assignmentId]);

  const handleNext = useCallback(() => {
    if (qIdx >= questions.length - 1) {
      const total    = questions.reduce((s, qi) => s + (qStates[qi.id]?.revealed ? qStates[qi.id].available : 0), 0);
      const maxTotal = questions.reduce((s, qi) => s + qi.scoring.points, 0);
      onComplete({
        answers: questions.map((qi) => {
          const st = qStates[qi.id];
          return { questionId: qi.id, raw: answers[qi.id], isCorrect: !!st?.revealed, points: st?.revealed ? st.available : 0 };
        }),
        totalScore: total, maxScore: maxTotal,
      });
    } else {
    ...
    <div className="qz-wrap">
      <div className="qz-progress-row">
        <div className="qz-progress-track">
          <div className="qz-progress-fill" style={{ width: `${(answered / questions.length) * 100}%`, background: color }} />
        </div>
        <span className="qz-progress-label">{answered} / {questions.length}</span>
        {answered > 0 && <span className="qz-score-running" style={{ color }}>{totalEarned} pts</span>}
      </div>
      <div className="qz-card">
        <div className="qz-card-header">
          ...
          <div className="qz-pts-group">
            {qs.available < q.scoring.points && <span className="qz-pts-orig">{q.scoring.points}</span>}
            <span className="qz-pts" style={{ color: qs.available < q.scoring.points ? (qs.available === 0 ? '#ef4444' : '#f59e0b') : undefined }}>
              {qs.available} pt{qs.available !== 1 ? 's' : ''} available
            </span>
          </div>
        </div>
        <p className="qz-stem">{q.stem}</p>
        {qs.forced    && <div className="qz-forced-banner">✗ Out of points — correct answer shown below</div>}
        {qs.lastWrong && !qs.forced && <div className="qz-wrong-banner">✗ Incorrect — {qs.available} pt{qs.available !== 1 ? 's' : ''} remaining. Try again.</div>}
        {qs.hintVisible && <div className="qz-hint-panel"><span className="qz-hint-label">Hint</span>{q.feedback?.reference}</div>}
        {q.payload.kind === 'multiple_choice' && <MultipleChoice q={q} shuffledOpts={shuffledOpts[qIdx]} selected={raw} onToggle={toggleOption} revealed={qs.revealed} forced={qs.forced} />}
        {q.payload.kind === 'true_false'      && <TrueFalse q={q} selected={raw} onSelect={updateAnswer} revealed={qs.revealed} forced={qs.forced} />}
        {q.payload.kind === 'drag_match'      && <DragMatch q={q} matchState={raw} onMatch={updateAnswer} revealed={qs.revealed} forced={qs.forced} partialFeedback={qs.partialFeedback} />}
        {q.payload.kind === 'fill_blank'      && <FillBlank q={q} value={raw} onChange={updateAnswer} revealed={qs.revealed} forced={qs.forced} />}
        {locked && (
          <div className={`qz-feedback ${qs.revealed ? 'qz-feedback-correct' : 'qz-feedback-wrong'}`}>
            <div className="qz-feedback-verdict">{qs.revealed ? `✓ Correct — +${qs.available} pts` : '✗ Answer revealed — 0 pts'}</div>
            <p className="qz-feedback-text">{qs.revealed ? q.feedback?.correct : q.feedback?.incorrect}</p>
      ...
      <div className="qz-actions">
        {locked ? (
          ...
          <>
            <button className="btn-submit" onClick={handleSubmit} disabled={!hasAnswer()}>Submit Answer</button>
            {hasHint && <button className="qz-hint-btn" onClick={handleHint}>💡 Use Hint <span className="qz-hint-cost">−1 pt</span></button>}
          </>
        )}
      </div>
    </div>
