...
/* ═══════════════════════════════════════════════════════════
   COHORT CONTROL PANEL — target reveal, cohort operations
═══════════════════════════════════════════════════════════ */

function CohortControlPanel({ cohorts, onCohortsChange }) {
  const [busy,   setBusy]   = useState({});
  const [errors, setErrors] = useState({});

  const toggleTargetReveal = async (cohort) => {
    const next = !cohort.target_revealed;
    setBusy((b) => ({ ...b, [cohort.id]: true }));
    setErrors((e) => ({ ...e, [cohort.id]: null }));
    try {
      const updated = await updateCohort(cohort.id, { target_revealed: next });
      onCohortsChange((prev) =>
        prev.map((c) => c.id === cohort.id ? { ...c, ...updated } : c)
      );
    } catch (err) {
      setErrors((e) => ({ ...e, [cohort.id]: err.response?.data?.error?.message ?? 'Update failed' }));
    } finally {
      setBusy((b) => ({ ...b, [cohort.id]: false }));
    }
  };

  return (
    <div style={{ padding: '20px 24px', maxWidth: 680 }}>
      <div style={{ fontFamily: 'var(--mono)', fontSize: 11, letterSpacing: '.14em', color: 'var(--primary)', marginBottom: 6, textTransform: 'uppercase' }}>
        OPERATION BRKR — COHORT OPERATIONS
      </div>
      <p style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 24, lineHeight: 1.6 }}>
        Control investigation target visibility per cohort. Revealing the target triggers the cinematic reveal sequence for all students in that cohort.
      </p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {cohorts.map((cohort) => {
          ...
          return (
            ...
            >
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--bright)', marginBottom: 3 }}>
                  {cohort.name}
                </div>
                <div style={{ fontFamily: 'var(--mono)', fontSize: 10, letterSpacing: '.12em', textTransform: 'uppercase', color: revealed ? '#10b981' : 'var(--muted)', display: 'flex', alignItems: 'center', gap: 6 }}>
                  {revealed ? (
                    <>
                      <span style={{ display: 'inline-block', width: 6, height: 6, borderRadius: '50%', background: '#10b981' }} />
                      TARGET REVEALED — students can see their investigation target
                    </>
                  ) : (
                    <>
                      <span style={{ display: 'inline-block', width: 6, height: 6, borderRadius: '50%', background: 'var(--muted)' }} />
                      TARGET CLASSIFIED — students see a redacted pending state
                    </>
                  )}
                </div>
                {errors[cohort.id] && (
                  <div style={{ fontSize: 11, color: '#ef4444', marginTop: 4 }}>{errors[cohort.id]}</div>
                )}
              </div>
