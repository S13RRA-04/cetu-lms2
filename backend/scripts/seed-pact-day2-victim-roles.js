'use strict';
/**
 * Seed PACKET HEIST v2 — Drop 2 ("The Pattern") per-victim, per-role
 * individual assessment work, each victim's squad quiz, and the cohort-wide
 * Cross-Squad Sync quiz, transcribed from "PACT Challenges"/
 * PACT_Day2_Squad_<Victim>_Student.docx (+ _InstructorKey.docx) and
 * PACT_Day2_CrossSquadSync_*.docx.
 *
 * Exact structural mirror of seed-pact-day1-victim-roles.js — see that file
 * for the rationale (dual-blank-as-prompt, role/squad-quiz shape). The only
 * addition here is the trailing Cross-Squad Sync spec, which has
 * victimName: null (not victim-scoped) — insertAssignment already handles a
 * null victimName fine since the column is nullable.
 *
...
const { SCENARIO, DROP, buildAssignmentSpecs } = require('./lib/day2VictimRoleSpecs');
  ...
  try {
    ...
    await seq.transaction(async (transaction) => {
      ...
      if (existing.length > 0) {
        ...
        if (subCount > 0) {
          throw new Error(`Refusing to replace Drop 2 v2 assignments — ${subCount} submission(s) already exist. Reseed manually if you're sure.`);
        }
        await seq.query('DELETE FROM assignments WHERE id IN (:ids)', { replacements: { ids }, transaction });
        console.log(`Deleted ${ids.length} prior untouched Drop 2 v2 assignment(s) before reseeding.`);
      }
    ...
    console.log(`Seeded Drop 2 v2 role + squad-quiz + cross-squad-sync assignments for 4 victims, all unpublished.`);
  } finally {
    await seq.close();
  }
}
