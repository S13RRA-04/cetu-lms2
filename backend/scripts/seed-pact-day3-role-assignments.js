'use strict';
/**
 * Seed PACKET HEIST v2 — Drop 3 ("The Broker") role-assessment work: 7
 * individual role assessments plus 1 cohort-wide Squad Synthesis Quiz,
 * transcribed from "PACT Challenges"/PACT_Day3_Role_Assignments_Student.docx
 * (+ _InstructorKey.docx).
 *
 * Structural mirror of seed-pact-day2-victim-roles.js — see that file and
 * day3RoleSpecs.js's header comment for why this drop is cohort-wide
 * (victim_name: null throughout) rather than per-victim like Day 1/Day 2.
 *
...
const { SCENARIO, DROP, buildAssignmentSpecs } = require('./lib/day3RoleSpecs');
  ...
  try {
    ...
    await seq.transaction(async (transaction) => {
      ...
      if (existing.length > 0) {
        ...
        if (subCount > 0) {
          throw new Error(`Refusing to replace Drop 3 v2 assignments — ${subCount} submission(s) already exist. Reseed manually if you're sure.`);
        }
        await seq.query('DELETE FROM assignments WHERE id IN (:ids)', { replacements: { ids }, transaction });
        console.log(`Deleted ${ids.length} prior untouched Drop 3 v2 assignment(s) before reseeding.`);
      }
    ...
    console.log(`Seeded Drop 3 v2 role + squad-synthesis-quiz assignments (cohort-wide), all unpublished.`);
  } finally {
    await seq.close();
  }
}
