'use strict';
/**
 * Seed PACKET HEIST v2 — Drop 4 ("The Case") role-assessment work: 7
 * individual role assessments plus 1 cohort-wide Squad Synthesis Quiz,
 * transcribed from "PACT Challenges"/PACT_Day4_Role_Assignments_Student.docx
 * (+ _InstructorKey.docx).
 *
 * Structural mirror of seed-pact-day3-role-assignments.js — see that file
 * and day4RoleSpecs.js's header comment for why this drop is cohort-wide
  ...
  try {
    ...
    await seq.transaction(async (transaction) => {
      ...
      if (existing.length > 0) {
        ...
        if (subCount > 0) {
          throw new Error(`Refusing to replace Drop 4 v2 assignments — ${subCount} submission(s) already exist. Reseed manually if you're sure.`);
        }
        await seq.query('DELETE FROM assignments WHERE id IN (:ids)', { replacements: { ids }, transaction });
        console.log(`Deleted ${ids.length} prior untouched Drop 4 v2 assignment(s) before reseeding.`);
      }
    ...
    console.log(`Seeded Drop 4 v2 role + squad-synthesis-quiz assignments (cohort-wide), all unpublished.`);
  } finally {
    await seq.close();
  }
}
